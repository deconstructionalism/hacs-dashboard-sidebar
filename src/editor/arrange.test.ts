...
const types = (blocks: SidebarConfig['header']): string[] =>
...
describe('moveBlock', () => {
  ...
  it('keeps the config valid after a move', () => {
    ...
    expect(validateSidebar(r)).toHaveLength(0);
  });
});
...
describe('factories', () => {
  it('produces valid default blocks for every type', () => {
    ...
    types_.forEach((t) => {
      ...
      expect(validateSidebar({ [region]: [defaultBlock(t)] }), t).toHaveLength(0);
    });
  });

  it('produces a valid starter sidebar and footer button', () => {
    expect(validateSidebar(starterSidebar())).toHaveLength(0);
    expect(
      validateSidebar({
